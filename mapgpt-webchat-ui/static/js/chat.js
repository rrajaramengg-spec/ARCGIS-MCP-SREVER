...
export function addAssistantMessage(text, rawData, queryId) {
    ...
    if (rawData?.action === 'query' && rawData?.data) {
        const normalized = normalizeQueryResponse(rawData.data);
    ...
    if (rawData?.action === 'analyze' && rawData?.data) {
    ...
    if (rawData?.action === 'locate') {
        // Uniform shape: data.source.location
        const loc = rawData?.data?.source?.location
    ...
    if (rawData && rawData.data && ['query', 'search', 'analyze'].includes(rawData.action)) {
...
export function buildFeatureTable(layerGroup) {
    ...
    // Add distance column for proximity results
    const hasDistance = features.some(f => f.distance != null);
    const distanceUnit = features[0]?.distance_unit || '';
    ...
    if (hasDistance) tableHtml += `<th>Distance (${escapeHtml(distanceUnit)})</th>`;
    columns.forEach(col => {
    ...
    displayFeatures.forEach(f => {
        tableHtml += '<tr>';
        if (hasDistance) {
            const d = f.distance != null ? f.distance.toFixed(2) : '';
            tableHtml += `<td>${escapeHtml(d)}</td>`;
        }
        columns.forEach(col => {
